...
- ยืนยัน claims จริงจาก userinfo ด้วย test account (โดยเฉพาะ `email` จาก scope `basic`) — รอ ticket 06 (client + test account) *(🆕 2026-09-07 คู่มือ OCS ชี้ว่า `basic` ไม่มี claim `email` ตรง ๆ — `mail` เฉพาะบุคลากร / `google-mail` / `office365-mail` + ชื่อ claim ไม่ตาม standard — ดู Amendment ใน [ticket 02](./tickets/02-sso-identity-to-local-user-mapping.md) และ [research/ku-playground-manual-notes.md](./research/ku-playground-manual-notes.md))*(🆕 2026-09-08: client + test account พร้อมแล้ว — เหลือรอแก้ `exchange` รองรับ `code_verifier` (PKCE enforced) ก่อน login จริงได้)*

...
class DiscountSeederTest extends TestCase
{
    ...
    private function createRoomTypeWithDailyRate(int $dailyRateBaht = 1200): RoomType
    {
        $rt = RoomType::create([
            'id' => Str::uuid(),
            'name_en' => 'Deluxe King',
    ...
    public function test_seeds_all_discount_types_and_quota_rules(): void
    {
        ...
        $this->assertSame(200, $save200->value);
        ...
        $this->assertSame(990, $deluxe990->value);
        ...
        $this->assertSame(100, $limited50->value);
    ...
    public function test_seeded_discounts_compute_expected_reductions(): void
    {
        ...
        // สมมติห้องราคา 1,200 บาท/คืน (1,200 baht) พัก 2 คืน = รวม 2,400 baht
        $rate = 1200;
        $nights = 2;

        // WELCOME10: 10% จาก 2,400 = 240 baht (240 บาท)
        $this->assertSame(240, $service->computeForRoom($welcome, $rate, $nights));

        // SAVE200: ลด 200 baht คงที่ต่อห้อง (200 บาท)
        $this->assertSame(200, $service->computeForRoom($save200, $rate, $nights));

        // DELUXE990: ลดส่วนต่าง (1,200 - 990 = 210 baht ต่อคืน) x 2 คืน = 420 baht (420 บาท)
        $this->assertSame(420, $service->computeForRoom($deluxe990, $rate, $nights));

        // LIMITED50: ลด 100 baht คงที่ต่อห้อง (100 บาท)
        $this->assertSame(100, $service->computeForRoom($limited50, $rate, $nights));
    }
